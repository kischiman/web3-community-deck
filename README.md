...
| Comments and proposals | Two different things, and comments come in two kinds. A comment left on the deck is **public** — shown there, and on `/team` marked as such. One left on `/team` is **team-only** and never reaches the deck, whatever the switch says. Which page it was written on decides, through separate routes, so it is a fact about how it arrived rather than something a caller asserts. A **proposal** carries a duration and a rate and lives on `/team` and the budget sheet. |
