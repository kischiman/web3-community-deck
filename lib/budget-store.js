...
function load() {
  try {
    ...
    if (Array.isArray(raw.tasks) && raw.rates) {
      raw.settings = raw.settings || { prefillRates: false };
...
/** Base amount from the rate card — admin's own estimate for the line. */
export function baseAmount(task, rates) {
  if (task.role === "fixed") return Number(task.qty) || 0;
  return (Number(task.qty) || 0) * (Number(rates[task.role]) || 0);
}

/** What the line costs once someone is on it; falls back to the base estimate. */
export function effectiveAmount(task, rates) {
  const p = proposalOf(task);
  if (!p) return baseAmount(task, rates);
...
export function totals(useEffective = true) {
  ...
  for (const t of tasks) byPhase[t.phase] += amount(t, rates);
...
export function publicView() {
  const prefill = state.settings.prefillRates;
  return {
    phases: PHASES,
    units: UNITS,
    prefillRates: prefill,
    // the rate card only crosses this line when the admin has opted in.
    // The contingency percentage is not a person's rate, so it travels either way.
    contingency: Number(state.rates.contingency) || 0,
    rates: prefill ? state.rates : null,
    tasks: state.tasks.map((t) => ({
      ...
      added: t.added,
      suggestedQty: t.qty,
      suggestedRate: prefill && t.role !== "fixed" ? state.rates[t.role] || null : null,
...
export function adminView() {
  return {
    ...
    tasks: state.tasks.map((t) => ({
      ...t,
      base: baseAmount(t, state.rates),
      effective: effectiveAmount(t, state.rates),
      proposals: t.proposals.map((p) => ({ ...p, amount: proposalAmount(p) })),
    })),
...
export function addTask({ phase, name, note, qty, role, unit }) {
  ...
  state.tasks.push({
    ...
    unit: UNITS.includes(unit) ? unit : role === "fixed" ? "fixed" : "days",
...
export function replaceState(next) {
  ...
  state = {
    ...
    settings: { prefillRates: false, ...(next.settings || {}) },
    tasks: next.tasks,
    updatedAt: Date.now(),
  };
  persist();
