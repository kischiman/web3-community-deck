...
function migrate(raw) {
  try {
    if (Array.isArray(raw.tasks) && raw.rates) {
      ...
      for (const t of raw.tasks) if (!Array.isArray(t.comments)) t.comments = [];
...
function buildView(force) {
  ...
  // Comments turned out to be working material too — questions between the people
  // costing this, and figures they are still arguing about. The public page takes them
  // and does not show them back; the team page is where they are read.
  const showComments = force ? force.comments : state.settings.publicComments === true;
  ...
  return {
    ...
    tasks: state.tasks.map((t) => {
      ...
      return {
        ...
        comments: showComments ? t.comments || [] : [],
