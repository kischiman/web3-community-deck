...
function seed() {
  ...
  return [
    ...
    s("p2a", "Present the findings from Phase 1", "Invite residents to validate and prioritise them; recruit the core groups", "researcher"),
    s("p2a", "Project identification", "Workshops to prioritise and select projects; neighbourhood walks and meetings", "designer"),
    s("p2a", "Co-design the intervention", "Educational sessions and training, then a design sprint for each group", "designer"),

    s("p2c", "Monitor and maintain engagement", "The data collection pipeline; sustaining usage momentum across both groups", "researcher"),
    s("p2c", "Complete the projects and prepare Phase 3", "Encourage completion; compile the records and gather feedback", "researcher"),
...
export async function init() {
  ...
  origin = source;
  state = foldIntoProcess(loaded ? migrate(loaded) : fresh());
  ...
  return {
    ...
    tasks: state.tasks.map((t) => {
      ...
      return {
        ...
        proposals: t.proposals.map((p) => publicProposal(p, money)),
        assigned: t.assigned,
      };
    }),
...
export function updateTask(id, { name, note, qty, unit, rate, memo }) {
  ...
  // Switching a line to a flat amount normally makes it an expense. A folded Delivery
  // line is the exception: it is a fixed amount *and* a fee, and the fold keeps expenses
  // out of its way — relabelling one on the first edit would be wrong twice over.
  if (task.unit === "fixed" && !task.folded) task.kind = "expense";

  save();
  return true;
}

export function addTask({ phase, name, note, qty, unit, rate, fromProcess, memo }) {
  if (!PHASES.some((p) => p.id === phase)) return false;
  const clean = String(name || "").trim().slice(0, 90);
  if (!clean) return false;
